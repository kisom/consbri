...
static double	read_brightnights(char *, char *);


/*
 * consbri is a console utility for setting the brightness on Linux systems.
 */
int 
main(int argc, char *argv[])
{
	...
	bright = read_brightness(backlight, BRIGHTNESS_FILE);
	if (1 == argc) {
		double		cur = 0.0;
		cur = read_brightness(backlight, MAX_BRIGHTNESS_FILE);
...
/*
 * get_backlight_class is used to determine the backlight class directory in
 * the sysfs filesystem. The caller is responsible for freeing the returned
 * character array.
 */
char           *
get_backlight_class()
...
/*
 * write brightness writes the desired brightness to the sysfs backlight
 * file.
 */
int
write_brightness(char *backlight, double brightness)
{
	...
	snprintf(fname, PATH_MAX, "%s/%s", backlight, BRIGHTNESS_FILE);
...
/*
 * get_brightness returns the brightness or max brightness
 */
double 
read_brightness(char *backlight, char *brightness_file)
{
